...
import { EditOrderRequestSchema } from "@/app/types/server/item";
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = EditOrderRequestSchema.safeParse(body);
  ...
  const { shopID, orderID, orderData, userID } = data;
  ...
  try {
    ...
    await orderDocRef.update(orderData);
